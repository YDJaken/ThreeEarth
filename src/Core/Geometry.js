...
Geometry.prototype = Object.assign( Object.create( EventDispatcher.prototype ), {
    ...
    merge: function ( geometry, matrix, materialIndexOffset ) {
        ...
        if ( ! ( geometry && geometry.isGeometry ) ) {
            ...
            console.error( 'Speed3D.Geometry.merge(): geometry not an instance of Speed3D.Geometry.', geometry );
            return;
    ...
    mergeMesh: function ( mesh ) {
        ...
        if ( ! ( mesh && mesh.isMesh ) ) {
            ...
            console.error( 'Speed3D.Geometry.mergeMesh(): mesh not an instance of Speed3D.Mesh.', mesh );
            return;
...
class Geometry extends EventDispatcher {
    ...
    fromBufferGeometry(geometry) {
        ...
        function addFace(a, b, c, materialIndex) {
            ...
            let vertexColors = colors !== undefined ? [that.colors[a].clone(), that.colors[b].clone(), that.colors[c].clone()] : [];
            let face = new Face3(a, b, c, vertexNormals, vertexColors, materialIndex);
            that.faces.push(face);
            if (uvs !== undefined) {
                that.faceVertexUvs[0].push([tempUVs[a].clone(), tempUVs[b].clone(), tempUVs[c].clone()]);
            }
            if (uvs2 !== undefined) {
                that.faceVertexUvs[1].push([tempUVs2[a].clone(), tempUVs2[b].clone(), tempUVs2[c].clone()]);
            }
        }
    ...
    merge(geometry, matrix, materialIndexOffset) {
        ...
        if (!(geometry && geometry.isGeometry)) {
            ...
            console.error('Speed3DEngine.Geometry.merge(): geometry not an instance of Speed3DEngine.Geometry.', geometry);
            return;
    ...
    mergeMesh(mesh) {
        ...
        if (!(mesh && mesh.isMesh)) {
            ...
            console.error('Speed3DEngine.Geometry.mergeMesh(): mesh not an instance of Speed3DEngine.Mesh.', mesh);
            return;
    ...
    toJSON() {
        ...
        data.data.normals = normals;
        if (colors.length > 0) data.data.colors = colors;
        if (this.uvs.length > 0) data.data.uvs = [this.uvs]; // temporal backward compatibility
        data.data.faces = faces;
        return data;
    }
    ...
    getNormalIndex(normal) {
        ...
        if (this.normalsHash[hash] !== undefined) {

            return this.normalsHash[hash];

        }

        this.normalsHash[hash] = this.normals.length / 3;
    ...
    getColorIndex(color) {
        ...
        if (this.colorsHash[hash] !== undefined) {

            return this.colorsHash[hash];

        }

        this.colorsHash[hash] = this.colors.length;
    ...
    getUvIndex(uv) {
        ...
        if (this.uvsHash[hash] !== undefined) {

            return this.uvsHash[hash];

        }

        this.uvsHash[hash] = this.uvs.length / 2;
