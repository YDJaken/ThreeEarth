...
import {Box3} from '../Datum/Math/Box3.js';
...
BufferGeometry.prototype = Object.assign( Object.create( EventDispatcher.prototype ), {
    ...
    setIndex: function ( index ) {
        ...
        if ( Array.isArray( index ) ) {
            ...
            this.index = new ( arrayMax.arrayMax( index ) > 65535 ? Uint32BufferAttribute : Uint16BufferAttribute )( index, 1 );
    ...
    addAttribute: function ( name, attribute ) {
        ...
        if ( ! ( attribute && attribute.isBufferAttribute ) && ! ( attribute && attribute.isInterleavedBufferAttribute ) ) {
            ...
            console.warn( 'Speed3D.BufferGeometry: .addAttribute() now expects ( name, attribute ).' );
        ...
        if ( name === 'index' ) {
            ...
            console.warn( 'Speed3D.BufferGeometry.addAttribute: Use .setIndex() for index attribute.' );
    ...
    setFromObject: function ( object ) {
        ...
        // console.log( 'Speed3D.BufferGeometry.setFromObject(). Converting', object, this );
    ...
    computeBoundingBox: function () {
        ...
        if ( isNaN( this.boundingBox.min.x ) || isNaN( this.boundingBox.min.y ) || isNaN( this.boundingBox.min.z ) ) {
            ...
            console.error( 'Speed3D.BufferGeometry.computeBoundingBox: Computed min/max have NaN values. The "position" attribute is likely to have NaN values.', this );
        ...
        return function computeBoundingSphere() {
            ...
            if ( position ) {
                ...
                if ( isNaN( this.boundingSphere.radius ) ) {
                    ...
                    console.error( 'Speed3D.BufferGeometry.computeBoundingSphere(): Computed radius is NaN. The "position" attribute is likely to have NaN values.', this );
    ...
    merge: function ( geometry, offset ) {
        ...
        if ( ! ( geometry && geometry.isBufferGeometry ) ) {
            ...
            console.error( 'Speed3D.BufferGeometry.merge(): geometry not an instance of Speed3D.BufferGeometry.', geometry );
            return;
        ...
        if ( offset === undefined ) {
            ...
            console.warn(
                'Speed3D.BufferGeometry.merge(): Overwriting original geometry, starting at offset=0. '
    ...
    toNonIndexed: function () {
        ...
        if ( this.index === null ) {
            ...
            console.warn( 'Speed3D.BufferGeometry.toNonIndexed(): Geometry is already non-indexed.' );
            return this;
...
class BufferGeometry extends EventDispatcher {
    ...
    addAttribute(name, attribute) {
        if (!(attribute && attribute.isBufferAttribute) && !(attribute && attribute.isInterleavedBufferAttribute)) {
            console.warn('Speed3DEngine.BufferGeometry: .addAttribute() now expects ( name, attribute ).');
            return this.addAttribute(name, new BufferAttribute(arguments[1], arguments[2]));
        }
        if (name === 'index') {
            console.warn('Speed3DEngine.BufferGeometry.addAttribute: Use .setIndex() for index attribute.');
            this.setIndex(attribute);
            return this;
        }
        this.attributes[name] = attribute;
    ...
    center() {
        ...
        let that = this
        return function center() {
            that.computeBoundingBox();
            that.boundingBox.getCenter(offset).negate();
            that.translate()(offset.x, offset.y, offset.z);
            return that;
        };
    }

    setFromObject(object) {
        // console.log( 'Speed3DEngine.BufferGeometry.setFromObject(). Converting', object, this );
    ...
    computeBoundingBox() {
        ...
        if (isNaN(this.boundingBox.min.x) || isNaN(this.boundingBox.min.y) || isNaN(this.boundingBox.min.z)) {
            console.error('Speed3DEngine.BufferGeometry.computeBoundingBox: Computed min/max have NaN values. The "position" attribute is likely to have NaN values.', this);
        }
    }
    ...
    computeBoundingSphere() {
        ...
        let that = this;
        return (function computeBoundingSphere() {
            if (that.boundingSphere === null) {
                that.boundingSphere = new Sphere();
            }
            let position = that.attributes.position;
            if (position) {
                let center = that.boundingSphere.center;
                box.setFromBufferAttribute(position);
                box.getCenter(center);
                // hoping to find a boundingSphere with a radius smaller than the
                // boundingSphere of the boundingBox: sqrt(3) smaller in the best case
                ...
                that.boundingSphere.radius = Math.sqrt(maxRadiusSq);
                if (isNaN(that.boundingSphere.radius)) {
                    console.error('Speed3DEngine.BufferGeometry.computeBoundingSphere(): Computed radius is NaN. The "position" attribute is likely to have NaN values.', that);
                }
            }
        })();
    }
    ...
    merge(geometry, offset) {
        if (!(geometry && geometry.isBufferGeometry)) {
            console.error('Speed3DEngine.BufferGeometry.merge(): geometry not an instance of Speed3DEngine.BufferGeometry.', geometry);
            return;
        }
        if (offset === undefined) {
            offset = 0;
            console.warn(
                'Speed3DEngine.BufferGeometry.merge(): Overwriting original geometry, starting at offset=0. '
                + 'Use BufferGeometryUtils.mergeBufferGeometries() for lossless merge.'
            );
        }
        let attributes = this.attributes;
    ...
    normalizeNormals() {
        ...
        let that = this;
        return function normalizeNormals() {
            let normals = that.attributes.normal;
    ...
    toNonIndexed() {
        if (this.index === null) {
            console.warn('Speed3DEngine.BufferGeometry.toNonIndexed(): Geometry is already non-indexed.');
            return this;
        }
