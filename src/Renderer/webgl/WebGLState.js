...
/*function WebGLState( gl, extensions, utils ) {
	...
	function compressedTexImage2D() {
		...
		try {
			...
			console.error( 'Speed3DEngine.WebGLState:', error );
...
class WebGLState {
    ...
    constructor(gl, extensions, utils) {
        ...
        this.depthBuffer = this._DepthBuffer();
        this.stencilBuffer = this._StencilBuffer();
        ...
        if (this.glVersion.indexOf('WebGL') !== -1) {
            this.version = Number.parseFloat(/^WebGL\ ([0-9])/.exec(this.glVersion)[1]);
            this.lineWidthAvailable = (this.version >= 1.0);
        } else if (this.glVersion.indexOf('OpenGL ES') !== -1) {
            this.version = Number.parseFloat(/^OpenGL\ ES\ ([0-9])/.exec(this.glVersion)[1]);
    ...
    _ColorBuffer() {
        ...
        let currentColorClear = new Vector4(0, 0, 0, 0);
        let that = this;
        return {
            setMask: function (colorMask) {
                if (currentColorMask !== colorMask && !locked) {
                    that.gl.colorMask(colorMask, colorMask, colorMask, colorMask);
                    currentColorMask = colorMask;
                }
            },
            setLocked: function (lock) {
    ...
    _DepthBuffer() {
        ...
        return {
            ...
            setTest: function (depthTest) {
                if (depthTest) {
                    scope.enable(gl.DEPTH_TEST);
                } else {
                    scope.disable(gl.DEPTH_TEST);
                }
            },
    ...
    _StencilBuffer() {
        ...
        return {
            ...
            setTest: function (stencilTest) {
                if (stencilTest) {
                    scope.enable(gl.STENCIL_TEST);
                } else {
                    scope.disable(gl.STENCIL_TEST);
                }
            },
