...
/*function WebGLPrograms( renderer, extensions, capabilities ) {
	...
	function allocateBones( object ) {
		...
		if ( capabilities.floatVertexTextures ) {
			...
			if ( maxBones < bones.length ) {
				...
				console.warn( 'Speed3DEngine.WebGLRenderer: Skeleton has ' + bones.length + ' bones. This GPU supports ' + maxBones + '.' );
				return 0;
	...
	function getTextureEncodingFromMap( map, gammaOverrideLinear ) {
		...
		if ( ! map ) {
			...
			console.warn( "Speed3DEngine.WebGLPrograms.getTextureEncodingFromMap: don't use render targets as textures. Use their .texture property instead." );
	...
	this.getParameters = function ( material, lights, shadows, fog, nClipPlanes, nClipIntersection, object ) {
		...
		if ( material.precision !== null ) {
			...
			if ( precision !== material.precision ) {
				...
				console.warn( 'Speed3DEngine.WebGLProgram.getParameters:', material.precision, 'not supported, using', precision, 'instead.' );
...
class WebGLPrograms {
    ...
    static _getTextureEncodingFromMap(map, gammaOverrideLinear) {
        let encoding;
        if (!map) {
            encoding = LinearEncoding;
        } else if (map.isTexture) {
    ...
    getParameters(material, lights, shadows, fog, nClipPlanes, nClipIntersection, object) {
        ...
        // heuristics to create shader parameters according to lights in the scene
        // (not to blow over maxLights budget)
        let maxBones = object.isSkinnedMesh ? this._allocateBones(object) : 0;
        ...
        return {
            ...
            outputEncoding: WebGLPrograms._getTextureEncodingFromMap((!currentRenderTarget) ? null : currentRenderTarget.texture, this.renderer.gammaOutput),
            map: !!material.map,
            mapEncoding: WebGLPrograms._getTextureEncodingFromMap(material.map, this.renderer.gammaInput),
            envMap: !!material.envMap,
            envMapMode: material.envMap && material.envMap.mapping,
            envMapEncoding: WebGLPrograms._getTextureEncodingFromMap(material.envMap, this.renderer.gammaInput),
            envMapCubeUV: (!!material.envMap) && ((material.envMap.mapping === CubeUVReflectionMapping) || (material.envMap.mapping === CubeUVRefractionMapping)),
            lightMap: !!material.lightMap,
            aoMap: !!material.aoMap,
            emissiveMap: !!material.emissiveMap,
            emissiveMapEncoding: WebGLPrograms._getTextureEncodingFromMap(material.emissiveMap, this.renderer.gammaInput),
            bumpMap: !!material.bumpMap,
            normalMap: !!material.normalMap,
            displacementMap: !!material.displacementMap,
            roughnessMap: !!material.roughnessMap,
