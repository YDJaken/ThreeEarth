...
Object.assign( Box3.prototype, {
	...
	getCenter: function ( target ) {
		...
		if ( target === undefined ) {
			...
			console.warn( 'Speed3DEngine.: .getCenter() target is now required' );
			target = new Vector3();
	...
	getSize: function ( target ) {
		...
		if ( target === undefined ) {
			...
			console.warn( 'Speed3DEngine.: .getSize() target is now required' );
			target = new Vector3();
	...
	getParameter: function ( point, target ) {
		...
		if ( target === undefined ) {
			...
			console.warn( 'Speed3DEngine.: .getParameter() target is now required' );
			target = new Vector3();
	...
	clampPoint: function ( point, target ) {
		...
		if ( target === undefined ) {
			...
			console.warn( 'Speed3DEngine.: .clampPoint() target is now required' );
			target = new Vector3();
	...
	getBoundingSphere: function () {
		...
		return function getBoundingSphere( target ) {
			...
			if ( target === undefined ) {
				...
				console.warn( 'Speed3DEngine.: .getBoundingSphere() target is now required' );
				target = new Sphere();
...
class Box3 {
    ...
    setFromCenterAndSize(center, size) {
        ...
        return (function setFromCenterAndSize() {
            ...
            that.min.copy(center).sub(halfSize);
            that.max.copy(center).add(halfSize);
            return that;
        })();
    }
    ...
    expandByObject(object) {
        ...
        let v1 = new Vector3();
        scope = this;
        return (function expandByObject() {
            object.updateMatrixWorld(true);
    ...
    intersectsSphere(sphere) {
        ...
        let that = this;
        return (function intersectsSphere() {
            // Find the point on the AABB closest to the sphere center.
            that.clampPoint(sphere.center, closestPoint);
            // If that point is inside the sphere, the AABB and sphere intersect.
            return closestPoint.distanceToSquared(sphere.center) <= (sphere.radius * sphere.radius);
        })();
    }
    ...
    intersectsTriangle(triangle) {
        ...
        return (function intersectsTriangle() {
            ...
            // compute box center and extents
            that.getCenter(center);
            extents.subVectors(that.max, center);
            // translate triangle to aabb origin
            v0.subVectors(triangle.a, center);
            v1.subVectors(triangle.b, center);
            v2.subVectors(triangle.c, center);
    ...
    distanceToPoint(point) {
        ...
        return (function distanceToPoint() {
            let clampedPoint = v1.copy(point).clamp(that.min, that.max);
            return clampedPoint.sub(point).length();
        })();
    }
    ...
    getBoundingSphere(target) {
        let v1 = new Vector3();
        let that = this;
        return (function getBoundingSphere() {
            if (target === undefined) {
                console.warn('Speed3DEngine.: .getBoundingSphere() target is now required');
                target = new Sphere();
            }
            that.getCenter(target.center);
            target.radius = that.getSize(v1).length() * 0.5;
            return target;
        });
    }
    ...
    toString(){
        return `min=${this.min.toArray().toString()},max=${this.max.toArray().toString()}`
    }
}
