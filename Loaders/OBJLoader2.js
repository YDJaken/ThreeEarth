...
import {DefaultLoadingManager} from "../src/Loaders/LoadingManager.js";

/**
 * Use this class to load OBJ data from files or to parse OBJ data from an arraybuffer
 * @class
 *
 * @param {Speed3D.DefaultLoadingManager} [manager] The loadingManager for the loader to use. Default is {@link Speed3D.DefaultLoadingManager}
 */
var OBJLoader2 = (function () {
	...
	function OBJLoader2( manager ) {
		console.info( 'Using Speed3D.OBJLoader2 version: ' + OBJLOADER2_VERSION );
	 ...
	 * @memberOf Speed3D.OBJLoader2
	 *
	 * @param {Speed3D.Object3D} streamMeshesTo Object already attached to scenegraph where new meshes will be attached to
	 */
	...
	/**
	 * Set materials loaded by MTLLoader or any other supplier of an Array of {@link Speed3D.Material}.
	 * @memberOf Speed3D.OBJLoader2
	 *
	 * @param {Speed3D.Material[]} materials Array of {@link Speed3D.Material}
	 */
	...
	/**
	 * Instructs loaders to create indexed {@link Speed3D.BufferGeometry}.
	 * @memberOf Speed3D.OBJLoader2
	 *
	 ...
	 * @memberOf Speed3D.OBJLoader2
	 *
	 * @param {Speed3D.LoaderSupport.PrepData} prepData All parameters and resources required for execution
	 * @param {Speed3D.LoaderSupport.WorkerSupport} [workerSupportExternal] Use pre-existing WorkerSupport
	 */
	...
	OBJLoader2.prototype.parseAsync = function ( content, onLoad ) {
		...
		var buildCode = function ( funcBuildObject, funcBuildSingleton ) {
			...
			workerCode += '  */\n\n';
			workerCode += 'Speed3D = { LoaderSupport: {} };\n\n';
			workerCode += funcBuildObject( 'Speed3D.LoaderSupport.Validator', Validator );
    ...
    var Parser = (function () {
        ...
        Parser.prototype.setMaterials = function ( materials ) {
            this.materials = Speed3D.LoaderSupport.Validator.verifyInput( materials, this.materials );
            this.materials = Speed3D.LoaderSupport.Validator.verifyInput( this.materials, {} );
        };

        Parser.prototype.setCallbackMeshBuilder = function ( callbackMeshBuilder ) {
            if ( ! Speed3D.LoaderSupport.Validator.isValid( callbackMeshBuilder ) ) throw 'Unable to run as no "MeshBuilder" callback is set.';
        ...
        Parser.prototype.checkSubGroup = function () {
            ...
            if ( ! Speed3D.LoaderSupport.Validator.isValid( this.rawMesh.subGroupInUse ) ) {
        ...
        Parser.prototype.buildFace = function ( faceIndexV, faceIndexU, faceIndexN ) {
            ...
            if ( this.useIndices ) {
                ...
                if ( Speed3D.LoaderSupport.Validator.isValid( indicesPointer ) ) {
        ...
        Parser.prototype.processCompletedMesh = function () {
            ...
            if ( Speed3D.LoaderSupport.Validator.isValid( result ) ) {
        ...
        Parser.prototype.buildMesh = function ( result ) {
            ...
            var haveVertexColors = Speed3D.LoaderSupport.Validator.isValid( colorFA );
            ...
            for ( var oodIndex in meshOutputGroups ) {
                ...
                if ( ! Speed3D.LoaderSupport.Validator.isValid( materialOrg ) && ! Speed3D.LoaderSupport.Validator.isValid( material ) ) {
                ...
                if ( ! Speed3D.LoaderSupport.Validator.isValid( material ) ) {
                ...
                if ( this.logging.enabled && this.logging.debug ) {
                    var materialIndexLine = Speed3D.LoaderSupport.Validator.isValid( selectedMaterialIndex ) ? '\n\t\tmaterialIndex: ' + selectedMaterialIndex : '';
            ...
            this.callbackMeshBuilder(
                ...
                [ vertexFA.buffer ],
                Speed3D.LoaderSupport.Validator.isValid( indexUA ) ? [ indexUA.buffer ] : null,
                Speed3D.LoaderSupport.Validator.isValid( colorFA ) ? [ colorFA.buffer ] : null,
                Speed3D.LoaderSupport.Validator.isValid( normalFA ) ? [ normalFA.buffer ] : null,
                Speed3D.LoaderSupport.Validator.isValid( uvFA ) ? [ uvFA.buffer ] : null
            );
        };
	...
	OBJLoader2.prototype._loadMtl = function ( resource, callbackOnLoad, crossOrigin, materialOptions ) {
		if ( MTLLoader === undefined ) console.error( '"Speed3D.MTLLoader" is not available. "Speed3D.OBJLoader2" requires it for loading MTL files.' );
